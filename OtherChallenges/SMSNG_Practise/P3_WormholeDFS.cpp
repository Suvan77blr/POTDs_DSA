...
There are N number of warmholes; each warmhole has 5 values. 
    First 2 values are starting co-ordinate of warmhole 
    After that value no. 3 and 4 represents ending co-ordinate of warmhole and 
    Last 5th value is represents cost to pass through this warmhole. 
    Now these warmholes are bi-directional. 
    
    Now the distance to go from (x1,y1) to (x2,y2) is abs(x1-x2)+abs(y1-y2). 
